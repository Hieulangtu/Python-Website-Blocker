...
label= Label(root, text ='WEBSITE BLOCKER' ,justify=CENTER,fg="red", font ='arial 15 bold').pack()
# Label(root, text ='HIEU-PROJECT' , font ='arial 20 bold').pack(side=BOTTOM)
#host_path = 'C:\Windows\System32\drivers\etc\hosts'

label2=Label(root, text ='Enter Website :' , font ='arial 14 bold').place(x=5 ,y=65)
...
WebsitesUn = Text(root,font = 'arial 10',height='2', width = '40', wrap = WORD, padx=5, pady=5).place(x= 180,y = 190)
    ...
    with open (hostsFile,'r+') as host_file:
        ...
        for website in Website:
                if website in file_content:
                    Label(root, text = 'Already Blocked' ,bg = 'red', font = 'arial 12 bold').place(x=290,y=120)
                    Button(root,text="exit",command=root.quit).pack(side=BOTTOM)
                    pass
                else:
                    host_file.write(ip_local + " " + website + '\n')
                    Label(root, text = "Blocked",bg = 'red', font = 'arial 12 bold').place(x=290,y =120)
                    Button(root,text="exit",command=root.quit).pack(side=BOTTOM)
        ...
        for websiteUn in Removedwebsites:      
            if websiteUn in file_content:
                ...
                # removing hostnmes from host file
                    ...
                    Label(root, text = 'Remove successfully' ,bg = 'red', font = 'arial 12 bold').place(x=280,y=250)
                    Button(root,text="exit",command=root.quit).pack(side=BOTTOM)
                pass #ko hieu sao lai o day
                    
            else:
                Label(root, text = "Not exit this file in blocked List",bg = 'red', font = 'arial 12 bold').place(x=280,y =250)
                Button(root,text="exit",command=root.quit).pack(side=BOTTOM)
...
#nut click chan
Button(root, text = 'Block',font = 'arial 12 bold',pady = 5,command = Blocker ,width = 6, bg = 'royal blue', activebackground = 'red').place(x = 200, y = 110)

#nut click remove chan
Button(root, text = 'UnBlock',font = 'arial 12 bold',pady = 5,command = UnBlocker ,width = 6, bg = 'royal blue', activebackground = 'red').place(x = 200, y = 240)
